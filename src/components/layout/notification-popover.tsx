...
import { faCheckDouble, faWandMagicSparkles, faTriangleExclamation, faCircleInfo } from "@fortawesome/free-solid-svg-icons";
                    ...
                    {sortedNotifications.length > 0 ? (
                        <div className="divide-y">
                            {sortedNotifications.map(notification => (
                                <div key={notification.id} className={cn("p-3 flex items-start gap-3", !notification.read && "bg-primary/5")}>
                                     {!notification.read && (
                                        <div className="w-2 h-2 rounded-full bg-primary mt-1.5 shrink-0"></div>
                                     )}
                                     <div className={cn("shrink-0 w-6 text-center", notification.read && "ml-4")}>
                                        <FontAwesomeIcon icon={typeIcons[notification.type]} className={cn("h-4 w-4", typeColors[notification.type])} />
                                     </div>
                                     <div className="flex-grow">
                                        <p className="text-sm font-medium">{notification.title}</p>
                                        <p className="text-xs text-muted-foreground">{notification.description}</p>
                                        <p className="text-xs text-muted-foreground/80 mt-1">{timeAgo(notification.createdAt.toDate())}</p>
                                     </div>
                                </div>
                            ))}
                        </div>
